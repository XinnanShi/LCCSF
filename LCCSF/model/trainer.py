...
from model.train_wrapper import LCCSFTrainWrapper
...
class Trainer:
    def __init__(self, cfg: DictConfig, stage_cfg: DictConfig, log: TensorboardLogger, run_path):
        ...
        self.LCCSF = nn.parallel.DistributedDataParallel(LCCSFTrainWrapper(cfg, stage_cfg).cuda(),
        ...
        self.log.log_string('model_size',
                            str(sum([param.nelement() for param in self.LCCSF.parameters()])))
        self.log.log_string(
            'number_of_parameters_that_require_gradient',
            str(
                sum([
                    param.nelement()
                    for param in filter(lambda p: p.requires_grad, self.LCCSF.parameters())
                ])))
        ...
        self.train()
        parameter_groups = get_parameter_groups(self.LCCSF, stage_cfg, print_log=(local_rank == 0))
    ...
    def do_pass(self, data, it=0):
        ...
        if self.use_amp:
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(self.LCCSF.parameters(), self.clip_grad_norm)
    ...
    def save_weights(self, it, save_copy=False):
        ...
        if save_copy:
            ...
            torch.save(self.LCCSF.module.state_dict(), model_path)
    ...
    def save_checkpoint(self, it, save_copy=False):
        ...
        checkpoint = {
            'it': it,
            'weights': self.LCCSF.module.state_dict(),
    ...
    def load_checkpoint(self, path):
        ...
        map_location = 'cuda:%d' % self.local_rank
        self.LCCSF.module.load_state_dict(weights)
    ...
    def load_weights_in_memory(self, src_dict):
        self.LCCSF.module.load_weights(src_dict)
    ...
    def weights(self):
        return self.LCCSF.module.state_dict()
